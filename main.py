...
def plot_spectrogram(signal_data, fs, title="Spectrogram", st_obj=None):
    f, t, Sxx = spectrogram(signal_data, fs=fs, nperseg=fs//2, noverlap=fs//4)
    fig, ax = plt.subplots()
    ax.pcolormesh(t, f, 10*np.log10(Sxx+1e-10), shading='gouraud')
    ax.set_ylabel('Frequency [Hz]')
    ax.set_xlabel('Time [sec]')
    ax.set_title(title)
    plt.colorbar(ax.images[0], ax=ax, label='Power/Frequency (dB/Hz)')
    if st_obj:
        st_obj.pyplot(fig)
    else:
        plt.show()
...
if experiment == "Intra-Subject Calibration Dependency":
    ...
    ax.legend()
    st.pyplot(fig)
    # Plot spectrograms (MISSING DELIVERABLE)
    st.subheader("Spectrogram (Time-Frequency) Comparison")
    st.write("Raw Signal Spectrogram:")
    plot_spectrogram(data[ch_idx, calib_time*fs:][:min_len], fs, f"Raw Spectrogram ({channel_names[ch_idx]})", st)
    st.write("Cleaned Signal Spectrogram:")
    plot_spectrogram(cleaned[ch_idx, :min_len], fs, f"Cleaned Spectrogram ({channel_names[ch_idx]})", st)
    ...
    summary = []
    bandpower_all = []
    # --- MISSING DELIVERABLE: Inter-subject matrix ---
    rrmse_matrix = pd.DataFrame(index=subject_names, columns=subject_names)
    blink_matrix = pd.DataFrame(index=subject_names, columns=subject_names)
    for test_subject in subject_names:
        ...
        # Fill inter-subject matrix (train: all but test_subject)
        for train_subject in subject_names:
            if train_subject == test_subject:
                rrmse_matrix.loc[train_subject, test_subject] = np.nan
                blink_matrix.loc[train_subject, test_subject] = np.nan
            else:
                # Train on train_subject only, test on test_subject
                d = subject_data[train_subject]
                d = np.array([preprocess_eeg(ch[np.newaxis, :], fs).flatten() for ch in d])
                calib_data_indiv = d[:, :30*fs]
                state_indiv = asr_calibrate(calib_data_indiv, fs)
                test_data_indiv = subject_data[test_subject]
                test_data_indiv = np.array([preprocess_eeg(ch[np.newaxis, :], fs).flatten() for ch in test_data_indiv])
                padded_indiv = np.hstack([np.zeros((test_data_indiv.shape[0], int(fs / 4))), test_data_indiv])
                cleaned_indiv = asr_process(padded_indiv, fs, state_indiv)
                min_len_indiv = min(test_data_indiv.shape[1], cleaned_indiv.shape[1])
                test_data_indiv = test_data_indiv[:, :min_len_indiv]
                cleaned_indiv = cleaned_indiv[:, :min_len_indiv]
                nblinks_o_indiv, nblinks_c_indiv = eye_blink_count(test_data_indiv, cleaned_indiv, fs)
                rrmse_indiv = np.mean([compute_rrmse(test_data_indiv[i], cleaned_indiv[i]) for i in range(test_data_indiv.shape[0])])
                rrmse_matrix.loc[train_subject, test_subject] = rrmse_indiv
                blink_matrix.loc[train_subject, test_subject] = nblinks_o_indiv - nblinks_c_indiv
        ...
        ax.legend()
        st.pyplot(fig)
        # Plot spectrograms for Ch_1
        st.write("Raw Signal Spectrogram (Ch_1):")
        plot_spectrogram(test_data[0], fs, f"Raw Spectrogram ({test_subject} Ch_1)", st)
        st.write("Cleaned Signal Spectrogram (Ch_1):")
        plot_spectrogram(cleaned[0], fs, f"Cleaned Spectrogram ({test_subject} Ch_1)", st)
    df_summary = pd.DataFrame(summary)
    st.dataframe(df_summary)
    st.subheader("Inter-Subject Cross-Validation Matrix (RRMSE)")
    st.dataframe(rrmse_matrix)
    st.subheader("Inter-Subject Cross-Validation Matrix (Blink Reduction)")
    st.dataframe(blink_matrix)
